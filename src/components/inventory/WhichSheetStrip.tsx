...
export default function WhichSheetStrip({ here }: { here: 'inventory' | 'batches' }) {
  ...
  const mine: React.CSSProperties = { ...box, borderColor: 'var(--accent)', background: 'var(--g-50)' };
  const name: React.CSSProperties = { fontSize: 13, fontWeight: 600, color: 'var(--n-900)', marginBottom: 2 };
  const body: React.CSSProperties = { fontSize: 12, color: 'var(--n-600)', lineHeight: 1.45 };
  ...
  return (
    <div style={{ border: '1px solid var(--n-200)', borderRadius: 8, background: 'var(--n-50)',
                  padding: 12, marginBottom: 16 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--n-900)', marginBottom: 8 }}>
        Which file changes what
      </div>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <div style={here === 'inventory' ? mine : box}>
          <div style={name}>
            Inventory sheet <span style={{ fontWeight: 400, color: 'var(--n-500)' }}>(inventory-….xlsx)</span>
          ...
          <div style={body}>
            ...
            {here !== 'inventory' && (
              <> <Link to="/panel/inventory" style={{ color: 'var(--accent)' }}>Open Inventory</Link>.</>
            )}
          </div>
        </div>
        <div style={here === 'batches' ? mine : box}>
          <div style={name}>
            Batches sheet <span style={{ fontWeight: 400, color: 'var(--n-500)' }}>(batches-….xlsx)</span>
          ...
          <div style={body}>
            ...
            {here !== 'batches' && (
              <> <Link to="/panel/inventory/batches" style={{ color: 'var(--accent)' }}>Open Batches &amp; Expiry</Link>.</>
            )}
          </div>
        </div>
      </div>
