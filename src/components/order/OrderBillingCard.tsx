...
interface Props {
  ...
  /** Credit notes raised against this order (`GET /orders/:id` → credit_notes). */
  creditNotes?: Array<{ id: string; cn_number: string; status: string; total?: number; total_minor?: number }> | null;
...
const OrderBillingCard: React.FC<Props> = ({
  ...
  manualInvoiceUrl, manualInvoiceFilename, manualInvoiceUploadedBy, creditNotes,
    ...
    <Card className="shadow-sm">
      ...
      <CardContent className="space-y-3 p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] font-medium text-slate-500">
              {invoiceNumberSource === 'manual'
                ...
                : invoiceNumber
                  ? 'Issued by this system — type over it to use your own.'
                  /* Corrected copy: a number is no longer produced by looking at
                     the invoice. Until somebody presses Issue (or sends it) the
                     PDF is a proforma and no number is consumed. */
                  : 'Not issued yet — use Invoice ▸ Issue tax invoice, or type your own here.'}
            </p>
          </div>
          <div>
        ...
        {/* ── CREDIT NOTES AGAINST THIS INVOICE ──────────────────────────────
            An issued invoice is never edited; a cancellation corrects it with a
            credit note. Without this the note existed only in a toast and in the
            Books panel, so the order that caused it showed no trace — and the
            invoice would look like it disagreed with the order for no reason. */}
        {(creditNotes?.length ?? 0) > 0 && (
          <div className="rounded-md border border-indigo-200 bg-indigo-50/60 p-2.5">
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-indigo-800">
              Credit notes against this order
            </p>
            <div className="space-y-1">
              {creditNotes!.map((cn) => (
                <div key={cn.id} className="flex items-baseline justify-between gap-2 text-xs">
                  <span className="font-mono font-medium text-indigo-900">{cn.cn_number}</span>
                  <span className="flex items-baseline gap-2">
                    <span className="capitalize text-indigo-700">{String(cn.status)}</span>
                    <span className="font-semibold tabular-nums text-indigo-900">
                      − ₹{(Number(cn.total ?? (Number(cn.total_minor) || 0) / 100)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </span>
                </div>
              ))}
            ...
            <p className="mt-1.5 text-[11px] leading-snug text-indigo-700">
              The invoice keeps the value it was issued at. What the customer owes is the
              invoice less these.
            </p>
          </div>
        )}
