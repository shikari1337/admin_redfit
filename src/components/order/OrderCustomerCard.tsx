...
const Stat: React.FC<{ label: string; value: React.ReactNode; hint?: string; tone?: 'default' | 'accent' }> = ({
  ...
  /* A row, not a tile: three tiles side by side are ~65px wide in the 20% rail
     and clipped every figure. */
  <div className="flex items-baseline justify-between gap-2 border-b border-slate-100 py-1.5 last:border-0">
    ...
    <span className="flex items-baseline gap-1.5 text-right">
      ...
      <span className={`text-base font-bold tabular-nums leading-tight ${tone === 'accent' ? 'text-emerald-700' : 'text-slate-900'}`}>
        {value}
      </span>
    </span>
  </div>
