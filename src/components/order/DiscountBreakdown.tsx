...
const DiscountBreakdown: React.FC<DiscountBreakdownProps> = ({ discounts, couponCode }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h2 className="text-base font-bold mb-2.5">Discounts Applied</h2>
      <div className="space-y-1.5">
        {discounts.map((discount, index) => (
          <div key={index} className="flex items-center gap-2 px-2 py-1.5 bg-green-50 rounded">
            <FaCheckCircle className="text-green-500" size={16} />
            <span className="text-sm text-gray-700">{discount}</span>
          </div>
        ))}
        {couponCode && (
          <div className="flex items-center gap-2 px-2 py-1.5 bg-blue-50 rounded mt-1">
